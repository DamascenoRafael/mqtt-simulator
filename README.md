...
* The key **TOPICS** has a array of objects where each one has the format:
    ...
    | `TIME_INTERVAL` | number |  Overwrites the broker level config value and applies only to this Topic | yes |
...
* The key **DATA** inside TOPICS has a array of objects where each one has the format:
    ...
    | `TYPE` | string | It can be `"int"`, `"float"`, `"bool"`, `"math_expression"`, `"raw_value"` | yes |
    ...
    | `VALUES` | array\<Objects|Array> | The values to be published in array order | if `TYPE` is `"raw_value"` |
