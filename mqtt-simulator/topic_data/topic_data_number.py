...
class TopicDataNumber(TopicData):
    ...
    def is_old_value_on_boundary(self):
        return self.old_value == self.data.get("MIN_VALUE") or self.old_value == self.data.get("MAX_VALUE")
    ...
    def generate_next_value(self):
        if self.data.get("RESTART_ON_BOUNDARIES", False) and self.is_old_value_on_boundary():
        ...
        if should_run_with_probability(1 - increase_probability):
            step *= -1
        return (
            max(self.old_value + step, self.data["MIN_VALUE"])
            if step < 0
            else min(self.old_value + step, self.data["MAX_VALUE"])
        )
